...
読み取り専用ではSQLの構文を解析してSELECT系以外・SELECT INTO・複数文・実行可能コメントを拒否し、MySQLセッションにも読み取り専用を設定します。SQLは1文ずつ自動コミットで実行します。セッション変更、明示的なトランザクション操作、CALLは未対応です。実行計画はSELECTに対するEXPLAINです。SQLの構文はMySQL向けパーサーが対応する範囲に限ります。

SQL結果の取得上限は1,000行、各値5,000文字（バイナリは最大2,500バイトを16進表示）、合計約5MBです。上限で省略した場合は画面に表示します。CSVは取得済みの結果を出力します。SELECTのサーバー側制限は5秒、SQL実行・取得の待機上限は15秒です。タイムアウト時、更新SQLの反映状況は別途確認が必要です。実行ごとにSQL専用の接続を破棄し、セッション状態がプレビュー処理へ残るのを防ぎます。
