...
接続ダイアログの「読み取り専用」を外すと、DBユーザーの権限の範囲で更新SQLも実行できます。設定は現在の接続に保持し、接続に失敗した場合は以前の接続と設定を維持します。現在は接続先を1つずつ切り替える構成で、接続情報をディスクに保存しません。成功した接続切り替え・切断時はクエリタブと履歴をリセットします。SQLの保存は明示的なファイル出力です。
